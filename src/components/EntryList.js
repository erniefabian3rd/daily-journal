...
export const EntryList = ({ moods, entries, entry_tags, tags, onEditButtonClick, onDeleteButtonClick }) => {
  ...
  useEffect(() => {
    if (searched_term !== "") {
      searchEntries(searched_term).then(
        entriesData => setEntries(entriesData))
    } else {
      setEntries(entries)
    }
